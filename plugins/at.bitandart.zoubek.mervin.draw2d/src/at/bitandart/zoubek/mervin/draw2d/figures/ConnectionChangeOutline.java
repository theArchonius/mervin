...
public class ConnectionChangeOutline extends Shape {
	...
	public Rectangle getBounds() {
		return pointsToCover.getBounds().expand(paddingWidth, paddingWidth);
	}
	...
	private int[] calculateOutlineShape(PointList pointsToCover, double offset, double lineWidth) {
		...
		for (int i = 0; i < pointsToCover.size(); i++) {
			...
			// no need to normalize as segmentDirection is already normalized
			...
			/*
			 * if there is no previous segment just copy the segment directions
			 */
			...
			/*
			 * outline shape point at the "left" (counterclockwise) side of the
			 * Vector P1 -> P2
			 */
			...
			if (perpLeft != 0) {
				...
				/*
				 * find the intersection point of the right outline lines of the
				 * current and the previous segment and use it as right point
				 */
			...
			/*
			 * outline shape point at the "right" (clockwise) side of the Vector
			 * P1 -> P2
			 */
			...
			if (i == 0) {
				...
				/*
				 * beginning of the connection -> also translate the outline in
				 * the opposite direction of the segment in order to apply the
				 * correct offset
				 */
				...
				/*
				 * end of the connection -> also translate the outline in the
				 * direction of the segment in order to apply the correct offset
				 */
	...
	public void setPointsToCover(PointList pointsToCover) {
		this.pointsToCover = pointsToCover;
		repaint();
	}
